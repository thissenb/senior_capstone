...
int main(int argc, char *argv[]) { //input should be opt_enc <input_file> key <port#>
    ...
    if (argc < 4 || argc > 4) { //check if correct number of inputs passed in
       fprintf(stderr,"otp_dec: usage %s hostname port\n", argv[0]);
       exit(0);
    }
	...
	//take off newline character before error checking an passing into otp_dec_d
	input[strlen(input)+1] = '\0';
	masterkey[strlen(masterkey)-1] = '\0';
	
	//need to add unusual character to input to parse input/key over in otp_dec_d
	...
	for(i = 0; i < (strlen(input)-1); i++){
		...
		if(strchr(compare, input[i]) == NULL){
			fprintf(stderr, "otp_dec: Input contains bad characters\n");
			exit(1);
		}
	}
    ...
    n = read(sockfd,buffer,74999); //read in the data sent from otp_dec_d
    ...
    printf("%s\n", buffer); //print out the text sent from otp_dec_d
